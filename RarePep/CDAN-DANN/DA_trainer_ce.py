...
class Trainer(object):
    def __init__(self,  seed,model, optim, device, train_dataloader, val_dataloader, test_dataloader, opt_da=None, discriminator=None,
        ...
        self.is_da = config["D"]["USE"]
        self.alpha = alpha
        self.n_class = config["DECODER"]["BINARY"]
        if opt_da:
            self.optim_da = opt_da
        if self.is_da:
            self.da_method = config["D"]["METHOD"]
            # self.domain_dmm = discriminator
            self.domain_classifier = discriminator
            if config["D"]["RANDOM_LAYER"] and not config["D"]["ORIGINAL_RANDOM"]:
                self.random_layer = nn.Linear(in_features=config["DECODER"]["IN_DIM"]*self.n_class, out_features=config["D"]
            ...
            elif config["D"]["RANDOM_LAYER"] and config["D"]["ORIGINAL_RANDOM"]:
                self.random_layer = RandomLayer([config["DECODER"]["IN_DIM"], self.n_class], config["D"]["RANDOM_DIM"])
                if torch.cuda.is_available():
                    self.random_layer.cuda()
            else:
                self.random_layer = False
        self.da_init_epoch = config["D"]["INIT_EPOCH"]
        self.init_lamb_da = config["D"]["LAMB_DA"]
        self.batch_size = config["SOLVER"]["BATCH_SIZE"]
        self.use_da_entropy = config["D"]["USE_ENTROPY"]
        ...
        self.original_random = config["D"]["ORIGINAL_RANDOM"]
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            auroc, auprc, f1, acc, val_loss = self.test(dataloader="val")
            if self.experiment:
            ...
            print('Validation at Epoch ' + str(self.current_epoch) + ' with validation loss ' + str(val_loss), " AUROC "
                  + str(auroc) + " AUPRC " + str(auprc) + " f1 " + str(f1) + " acc " + str(acc))
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            ...
            if self.current_epoch >= self.da_init_epoch:  # 源域先训练多少轮之后，再进行目标域训练
                ...
                if self.da_method == "CDAN":
                    ...
                    reverse_f = ReverseLayerF.apply(f, self.alpha)  # (32,256)
                    reverse_f_t = ReverseLayerF.apply(f_t, self.alpha)  # (32,256)
                    # 域分类器（二分类：源域 vs 目标域）
                    result_reverse = torch.cat((reverse_f,reverse_f_t), dim=0)
                    domain_pred = self.domain_classifier(result_reverse)
                    domain_n,da_loss = cross_entropy_logits(domain_pred, domain_labels)
                else:
