...
def main():
    aa = [12]
    best_epoch = [164]
    # bacteria_fungi_virus:[151,152,196,182,125] bacteria_fungi:[188,187,175,180] bacteria_virus:[193,49,191,49,87] fungi_virus:[164,177,182,157,125]
    for seed, current_epoch in zip(aa, best_epoch):
        ...
        dataFolder = '/mnt/402f169c-136d-4c4c-b598-a9732ee96752/home/zxp/zhouxiaoping/kuayu/duoyu'

        if not cfg.DA.TASK:
            train_path = os.path.join(dataFolder, str(seed), 'B+V.xlsx')
            val_path = os.path.join(dataFolder, str(seed), 'Parasite.xlsx')
            test_path = os.path.join(dataFolder, str(seed), 'Parasite.xlsx')
            ...
            # 加载 BERT 模型和分词器
            bert_model_name = "/mnt/402f169c-136d-4c4c-b598-a9732ee96752/home/zxp/zhouxiaoping/BioBert"  # BERT 模型名称
            tokenizer_bert = AutoTokenizer.from_pretrained(bert_model_name)
            model_bert = AutoModel.from_pretrained(bert_model_name)
            model_bert = model_bert.eval().to(device)

            # 加载 ESM 模型和分词器
            esm_model_name = "/mnt/402f169c-136d-4c4c-b598-a9732ee96752/home/zxp/zhouxiaoping/esm"  # ESM 模型名称
        ...
        #     torch.load(
        #         f"/home/zhouxiaoping/jianmoxing/DrugBAN-main/DrugBAN-main/result/text-transfer/fungi+virus-parasite/{seed}/best_model_epoch_{current_epoch}.pth"))
