...
class DrugBAN(nn.Module):
    ...
    def forward(self, bg_d, v_p, mode="train"):
        ...
        # for i in range(2):  # 2层
        #     v31, src = self.protein_extractor(src)  # (64,640,128)
        #     talkingsrc = torch.add(talkingsrc, v31)  # 在第一个维度上进行拼接  (64,640,128)
        #     t_talkingsrc = talkingsrc.permute(0, 2, 1)
        #     conved_t = self.conv(self.dropout(t_talkingsrc))
        ...
        f, att = self.bcn(trg, src)  # f:(32,256), att:(32,2,290,1185)
        ...
        # src = self.fc(protein)  # (64,16,64)-->(64,16,128)
        # trg = self.ft(trg)  # (64,24,32)-->(64,24,128)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, conv_input, usesrc):  # , usesrc
        ...
        # # feed forward
        conv_input = self.ln(conv_input + self.do(self.sa(conv_input, conv_input, conv_input)))
        conv_input = conv_input.permute(0, 2, 1)  # (B,128,24)
        conv_input = self.conv_net(conv_input).permute(0, 2, 1)
